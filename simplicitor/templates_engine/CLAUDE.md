...
**Hard stop.** A deck cannot be templated when every one of its layouts lacks a fillable content placeholder, meaning one with `1 <= idx < 10` (not the title at idx 0, not custom at idx >= 10) and a type outside `{DATE, FOOTER, SLIDE_NUMBER}`. The predicate reads `prs.slide_layouts` only (`inspect_pptx` then `score_layouts`); the deck's own slides are never examined, so what the slides contain does not affect the outcome. This is a normal returned result (`status: "hard_stop"` plus a verbatim user-facing message), not an exception. The message in `breakdown.py` still attributes the cause to the deck's slides and has not been corrected.
