...
"From template..." in the Create panel (enabled once a model is connected) opens `TemplateDialog`, a modal `QStackedWidget` state machine: SELECTION -> CONFIRM -> PREVIEW -> DONE, with a `HardStopDialog` branch off the upload path. Generation runs off the UI thread via a MainWindow-owned `TemplateGenerateWorker` (moveToThread pattern; `started` / `progress` / `completed` / `failed` signals). Import and render run synchronously under a wait cursor; only generation is threaded. PREVIEW shows the validated content as editable JSON, re-validated before a synchronous render. Every failure surfaces through the existing inline status or dialog pattern, never a raw traceback. The dialog blocks its own dismissal while a generation is in flight.
